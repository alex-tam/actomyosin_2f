...
function optimise_network(s_old, af, xl, mm, parN, parA, parM, Lxx, Lxy, Lyx, Lyy)
    ...
    @time result = optimize(x -> energy_functional(x, s_old, af, xl, mm, parN, parA, parM, Lxx, Lxy, Lyx, Lyy), dof, LBFGS(alphaguess = LineSearches.InitialHagerZhang(α0 = 1.0)), Optim.Options(x_tol = parN.xTol, f_tol = parN.fTol, g_tol = parN.gTol); autodiff = :forward);
